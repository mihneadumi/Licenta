...
# Attempt to import the multi-threaded CPU matrix multiplication function
try:
    # Adjust path if necessary
    from algorithms.matrix_multiplication.multi_thread import multi_threaded_multiply
except ImportError:
    print("Error: Could not import 'multi_threaded_multiply' from 'algorithms.matrix_multiplication.multi_thread'.")
    print("Please ensure the file and function exist and are named correctly.")
    # Define a dummy function if import fails
    def multi_threaded_multiply(A, B):
        print("Warning: Using dummy multi_threaded_multiply function.")
        # Mimic workload roughly - use numpy's matmul which might be multi-threaded itself
        # or just return zeros. Returning zeros is safer for profiling comparison.
...
def generate_matrices(size):
    ...
    # Using float64 as in the GPU profiler for consistency
    A = np.random.rand(size, size).astype(np.float64)
    B = np.random.rand(size, size).astype(np.float64)
    return A, B

def profile_cpu_multi(A_np: np.ndarray, B_np: np.ndarray):
    """
    Profiles the execution time of multi-threaded CPU matrix multiplication.
    ...
    """
    try:
        # Time the CPU multi-threaded multiplication
        start_cpu = time.time()
        C_np = multi_threaded_multiply(A_np, B_np)
        end_cpu = time.time()
        # No explicit sync needed for CPU timing like with CUDA kernels

        # Optional: Verify result correctness here if needed, outside timing

        return end_cpu - start_cpu
    except Exception as e:
        print(f"  Error during CPU multi-threaded profiling: {e}")
        traceback.print_exc() # Print traceback for unexpected errors
        return float('inf')

def save_cpu_multi_stats(size: int, runs: int):
    """
    Runs multi-threaded CPU matrix multiplication profiling for a given size
    ...
    """
    print(f"Info: Profiling Multi-threaded CPU Matrix Multiplication for size {size}x{size}. Running {runs} times...")
    ...
    file_path = os.path.join(output_dir, 'cpu_multi_thread_stats.txt') # Specific filename
    ...
    try:
        ...
        try:
            ...
            print(f"Info: CPU Cores: {physical_cores} physical, {logical_cores} logical")
        except Exception as e:
        ...
        with open(file_path, 'w') as file:
            ...
            for run_number in range(1, runs + 1):
                ...
                # Profile the CPU multi-threaded multiplication
                multiply_time = profile_cpu_multi(A_np, B_np)
                ...
                # Print run result to console, matching GPU style
...
def run_matrix_mult_cpu_multi_threaded_prof(runs: int = 10, size: int = 1000):
    """
    Helper function to start the multi-threaded CPU matrix multiplication profiling.
...
if __name__ == "__main__":
    ...
    run_matrix_mult_cpu_multi_threaded_prof(runs=10, size=1000)
