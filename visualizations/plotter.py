...
def get_implementation_sort_key(impl_name):
    ...
    if 'gpu' in name_lower:
        return (2, impl_name)  # GPU implementations
    # Keywords for multi-threaded/parallel CPU
    elif any(keyword in name_lower for keyword in ['multi_thread', 'multi-thread', 'parallel', 'numba']):
        # Handle cases like 'numba_single_thread' if it should be single-threaded
        if 'numba' in name_lower and ('single_thread' in name_lower or 'sequential' in name_lower):  # Made more robust
            return (0, impl_name)  # Explicitly single-threaded numba
        return (1, impl_name)  # Multi-threaded/Parallel CPU implementations
    # Default to single-threaded CPU if no other keywords match
    # Catches names like 'cpu_single_thread', 'sequential', 'baseline_cpu', etc.
    elif any(keyword in name_lower for keyword in ['single_thread', 'single-thread', 'sequential']):
        return (0, impl_name)
    else:  # Default to single-threaded if no other specific markers
        # This could catch generic names like 'cpu_implementation'.
        # Consider if more specific single-thread keywords are needed if this is too broad.
        # For now, if not GPU and not explicitly multi/parallel, assume single.
        return (0, impl_name)
...
def load_data_file(file_path):
    ...
    try:
        ...
        perf_col = None
        # Check for known performance metric columns
        ...
        if perf_col:
            ...
            if (df[perf_col] <= 0).any():
                # print(f"Warning: Non-positive performance values found in {file_path} for {perf_col}. Filtering them out for log scale compatibility.")
                df = df[df[perf_col] > 0]
        ...
        if df.empty:
            # print(f"Warning: DataFrame became empty after processing {file_path}. Skipping.")
            return None
        return df
...
def plot_individual_run(df, algorithm, size_str, implementation, output_dir):
    ...
    if pd.notna(time_stdev) and time_stdev > 1e-9:  # Only show stdev if meaningful
    ...
    if perf_col and perf_col in df_plot.columns and not df_plot[perf_col].empty:
        ...
        if pd.notna(perf_stdev) and perf_stdev > 1e-9:  # Only show stdev if meaningful
...
def plot_comparison(stats_dict, metric_name, unit, algorithm, size_str, output_dir, use_median=False):
    ...
    # Sort original implementation keys (these are the keys for stats_dict)
    ...
    values = [v if pd.notna(v) and v > 0 else float('nan') for v in values]  # For log scale
    errors = [stats_dict[impl_key].get(metric_name, {}).get('stdev', float('nan')) for impl_key in sorted_impl_keys]

    # Filter out implementations with NaN values for this metric
    valid_indices = [i for i, v in enumerate(values) if pd.notna(v)]
    if not valid_indices: return

    # Create lists based on valid, sorted data
    # filtered_impl_keys are the original names, used to fetch data
    ...
    # Single-threaded implementations will be labeled 'single_threaded_stats'
    display_labels_for_plot = []
    for impl_key in filtered_impl_keys:  # Iterate over the actual keys that have valid data
        sort_category, _ = get_implementation_sort_key(impl_key)
        if sort_category == 0:  # Category 0 is single-threaded
            display_labels_for_plot.append('single_threaded_stats')
        else:
            display_labels_for_plot.append(impl_key)  # Use original name for multi-threaded and GPU
    ...
    # Set xticks using numerical positions and then the generated display labels
    ...
    for i, bar in enumerate(bars):  # bar objects are now indexed 0, 1, 2...
        yval = bar.get_height()
        ...
        # bar.get_x() will give the left edge of the bar (which is a numerical position)
    ...
    if max_value_plotted > 0:
        ...
        if max_text_y_calculated >= current_top_lim * 0.85:
            new_top_lim = max_text_y_calculated * 1.25  # Increased padding slightly
...
for algorithm, sizes_dict in all_data.items():
    for size_str_key, implementations_dict in sizes_dict.items():
        ...
        if gpu_impl:
            ...
            # If still no specific single-thread, try broader CPU terms
            if not cpu_baseline_impl:
            ...
            # Final fallback: any non-GPU implementation, prefer category 0
            if not cpu_baseline_impl:
